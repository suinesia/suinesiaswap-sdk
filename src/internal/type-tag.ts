...
export class StructTag {
  ...
  getShortAddressFullName() : string {
    ...
    return `${this.address}::${this.module}::${
        this.name
    }${typeParamString}`;
  }

  getParamlessName(): string {
    return `${this.address}::${this.module}::${this.name}`;
  }
...
export function parseQualifiedStructTag(
  ...
  const hexAddress = normalizeSuiObjectId(address);
...
export function parseMoveStructTag(moveTag: string): StructTag {
